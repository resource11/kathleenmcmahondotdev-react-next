---
slug: smoothly-inclusive-component-documentation
date: 2021-03-08
title: "Smoothly Inclusive Component Library Documentation"
author: "Kathleen McMahon"
excerpt: "Button Buttons allow users to perform single-click actions such as adding, deleting, or submitting an event. Editable fancy code Behold..."
tags:
  - accessibility
  - css
  - inclusive design
  - gatsby
  - mdx
isPublished: true
isHidden: false
isFeatured: false
---

import { Image } from "astro:assets";
import dayLilies from "./daylilies-in-july.jpg";

import {
  Button,
  Card,
  CTALink,
  DisplayBox,
  Icon,
  Link,
} from "../../../../../src/components";

## Button

Buttons allow users to perform single-click actions such as adding, deleting, or submitting an event.

<Image
  src={dayLilies}
  alt="A very cool penguin!"
  style="height: 100%; width: 100%; object-fit: cover;"
/>

---

### Editable fancy code

Behold! Pass `react-live` as a prop to your `jsx` codeblock and edit away.

```jsx react-live
<Button>Click me</Button>
```

---

### Syntax highlighting

You can have nice syntax highlighting with `prism-react-renderer`

<p></p>

```jsx
<Button>Click me</Button>
```

---

### Non-editable code

Pair your static code with a component by passing the `displaybox` prop to the `jsx` codeblock.

```jsx displaybox
<Button>Click me</Button>
```

---

If you haven't yet, head on over and check out [the excellent TECH(K)NOW Day lineup](https://www.techknowday.com/).
